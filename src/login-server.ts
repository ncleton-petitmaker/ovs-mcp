...
export async function startLoginServer(client: OvsClient): Promise<LoginFlow> {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.url !== expected)
      return send(
        response,
        404,
        page("Page introuvable", "Cette page de connexion n’est pas valide."),
      );
    if (request.method === "GET") return send(response, 200, formPage());
    if (request.method !== "POST")
      return send(
        response,
        405,
        page("Méthode refusée", "Revenez à la page de connexion."),
      );
    if (++attempts > 5)
      return send(
        response,
        ...
        page(
          "Trop de tentatives",
          "Fermez cette page puis relancez la connexion depuis votre client MCP.",
        ),
      );
    try {
      ...
      send(
        ...
        page(
          "Connexion réussie",
          "Vous pouvez fermer cette page et revenir dans votre client MCP.",
        ),
      );
      settle();
...
async function readBody(request: IncomingMessage): Promise<string> {
  ...
  for await (const chunk of request) {
    body += String(chunk);
    if (body.length > 16_384) throw new Error("Login form is too large.");
  }
  return body;
}

function formPage(error = ""): string {
