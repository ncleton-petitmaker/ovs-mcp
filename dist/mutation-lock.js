...
export class MutationFileLock {
    ...
    constructor(sessionPath, options = {}) {
        ...
        this.#acquireTimeoutMs = positiveDuration(options.acquireTimeoutMs, DEFAULT_ACQUIRE_TIMEOUT_MS, "acquire timeout");
        this.#staleAfterMs = positiveDuration(options.staleAfterMs, DEFAULT_STALE_AFTER_MS, "stale threshold");
        this.#pollIntervalMs = positiveDuration(options.pollIntervalMs, DEFAULT_POLL_INTERVAL_MS, "poll interval");
    }
    async run(operation) {
    ...
    async #acquire() {
        ...
        while (true) {
            ...
            if (performance.now() >= deadline) {
                throw new OvsError("Another OVS cart mutation still owns this private session. Retry after it finishes; if the process crashed, wait for stale-lock recovery instead of deleting runtime files manually.", "OVS_CART_MUTATION_LOCK_TIMEOUT", true);
            }
    ...
    #release(owner) {
        ...
        if (!current ||
            ...
            throw new OvsError("OVS cart mutation lock ownership changed before release.", "OVS_CART_MUTATION_LOCK_LOST");
        }
    ...
    #recoverMissingOwner() {
        ...
        if (!recovery) {
            if (ownerFileExists(this.#lockPath)) {
                ...
                if (latestIdentity.dev !== identity.dev ||
                    latestIdentity.ino !== identity.ino) {
                    return;
                }
            ...
            try {
                ...
                writeFileSync(ownerPath(this.#lockPath), `${JSON.stringify(recovery)}\n`, { encoding: "utf8", mode: 0o600, flag: "wx" });
            }
            catch (error) {
...
export function mutationLockPath(sessionPath) {
...
function readMissingOwnerRecovery(lockPath) {
    try {
        const value = JSON.parse(readFileSync(ownerPath(lockPath), "utf8"));
        if (!value || typeof value !== "object" || Array.isArray(value))
            return null;
        ...
        if (recovery.version !== 1 ||
            ...
            !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(recovery.token) ||
...
function ownerFileExists(lockPath) {
    try {
