...
export function createServer(options) {
    ...
    server.registerTool("connect_ovs", {
    ...
    }, async ({ harPath }) => result(async () => {
        if (harPath)
            await importSessionHar(harPath, options.sessionPath);
        const exists = await stat(options.sessionPath).then((value) => value.isFile(), () => false);
        if (!exists) {
            return {
                data: {
                    status: "connection_required",
                    sessionPath: options.sessionPath,
                    userAction: "Capture one authenticated OVS app session as HAR on a device you control, then call connect_ovs again with its absolute local path.",
                    privacy: "Keep the HAR local. Never upload it to GitHub, chat, an issue, or a model provider. Restore the phone proxy after capture.",
                },
            };
        }
        ...
        return {
            data: {
                ...
                session: publicSessionSummary(await loadSession(options.sessionPath)),
            },
        };
    }));
    ...
    registerReadTool(server, "list_categories", "List OVS categories", "List active catalog categories.", () => service.listCategories());
    registerReadTool(server, "list_manufacturers", "List OVS manufacturers", "List active brands that currently have products.", () => service.listManufacturers());
    ...
    registerReadTool(server, "list_currencies", "List OVS currencies", "List currencies exposed by OVS.", () => service.listCurrencies());
    registerReadTool(server, "get_cart", "Get OVS cart", "Read the authenticated account cart.", () => service.getCart());
    registerReadTool(server, "get_customer", "Get OVS customer", "Read the authenticated customer profile. This can contain personal data.", () => service.getCustomer());
    registerReadTool(server, "list_addresses", "List OVS addresses", "Read delivery addresses for the authenticated account. This returns personal data.", () => service.listAddresses());
    registerReadTool(server, "list_favorites", "List OVS favorites", "Read favorite products.", () => service.listFavorites());
    return server;
}
